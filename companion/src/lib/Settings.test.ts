 ...
 *
 * onMount registers the cross-window event listeners through a dynamic
 * `import("@tauri-apps/api/event")`; awaiting that module here guarantees its
 * `.then` has run before assertions, regardless of how many microtask hops the
 * mocked import takes on the installed vitest. */
