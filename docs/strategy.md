...
aiui ist ein Kanal, über den Coding-Agenten auf dem Rechner des Users echte UI rendern — strukturierte Eingaben, die im Chat zu unhandlich oder zu fehleranfällig wären. Topologie heute: Tauri-Companion auf dem Rechner des Users — macOS, seit 0.10.1 auch Windows (x64) —, der einen host-agnostischen MCP-/HTTP-Kontrakt auf `localhost:7777` exponiert. Der Companion **registriert sich selbst** bei den unterstützten Hosts (heute Claude Desktop und Claude Code) — ein Eintrag, der auf denselben gebündelten `--mcp-stdio`-Server zeigt; der User trägt nichts manuell ein. Remote-Agenten (Claude Code via SSH) erreichen den Companion über Reverse-Tunnel und POSTen Dialog-Specs an `localhost:7777`. Codex/ChatGPT wird als first-class Host **auf demselben Weg** angebunden — aiui trägt sich self-contained in Codex ein, genau wie bei Claude, statt den User `~/.codex/config.toml` von Hand editieren zu lassen (in Arbeit, #158). Genau diese Host-Agnostik ist der Hebel: dieselbe Capability ist über jeden MCP-fähigen Host portabel, ohne je gegen einen einzelnen Client zu bauen.
